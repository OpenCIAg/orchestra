...
@Component({
  ...
  template: `<div class="orc-segmented-control" [class]="'orc-segmented-control ' + styleClass()" role="group" [attr.aria-label]="label() || null">@for (option of options(); track option.value) { <button type="button" [disabled]="disabled() || cvaDisabled() || readonly() || option.disabled" [class.orc-segmented-control__option--selected]="isSelected(option)" [attr.aria-pressed]="isSelected(option)" (click)="select(option)">@if(option.icon){<span aria-hidden="true">{{option.icon}}</span>}{{option.label}}</button> }</div>`,
...
export class SegmentedControlComponent<T = unknown> implements ControlValueAccessor {
  readonly options = input<SegmentedControlOption<T>[]>([]); readonly value = model<T | null>(null); readonly label = input<string | undefined>(undefined); readonly styleClass = input(''); readonly disabled = input(false, { transform: booleanAttribute }); readonly readonly = input(false, { transform: booleanAttribute }); readonly change = output<T>(); readonly onChange = output<{ value: T }>();
