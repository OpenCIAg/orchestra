...
@Component({
  ...
  template: `<span class="orc-tag" [class]="'orc-tag orc-tag--' + effectiveVariant() + ' ' + styleClass()" [class.orc-tag--rounded]="rounded()" [attr.aria-disabled]="disabled() || null">@if (icon()) { <span aria-hidden="true">{{ icon() }}</span> }<span>{{ value() ?? label() }}</span>@if (removable()) { <button type="button" [disabled]="disabled()" [attr.aria-label]="'Remove ' + (value() ?? label())" (click)="remove($event)">×</button> }</span>`,
...
export class TagComponent {
  readonly label = input('Tag'); readonly value = input<string | undefined>(undefined); readonly variant = input<TagVariant>('neutral'); readonly severity = input<TagVariant | undefined>(undefined); readonly icon = input(''); readonly rounded = input(false, { transform: booleanAttribute }); readonly removable = input(false, { transform: booleanAttribute }); readonly disabled = input(false, { transform: booleanAttribute }); readonly styleClass = input('');
