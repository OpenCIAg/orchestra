...
export class ColorPickerComponent implements ControlValueAccessor {
  ...
  private parseColor(color: string): { r: number; g: number; b: number } | null { const value = color.trim(); const hex = value.match(/^#([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6})$/); if (hex) { const raw = hex[1].length === 3 ? hex[1].split('').map(char => char + char).join('') : hex[1]; return { r: parseInt(raw.slice(0, 2), 16), g: parseInt(raw.slice(2, 4), 16), b: parseInt(raw.slice(4, 6), 16) }; } const rgb = value.match(/^rgba?\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*\)$/i); if (rgb) { const channels = rgb.slice(1, 4).map(Number); return channels.every(channel => channel <= 255) ? { r: channels[0], g: channels[1], b: channels[2] } : null; } const hsv = value.match(/^(?:hsv|hsb)\(\s*(\d+(?:\.\d+)?)\s*,\s*(\d+(?:\.\d+)?)%?\s*,\s*(\d+(?:\.\d+)?)%?\s*\)$/i); if (hsv) { const h = Number(hsv[1]) / 60; const s = Number(hsv[2]) / 100; const v = Number(hsv[3]) / 100; const c = v * s; const x = c * (1 - Math.abs((h % 2) - 1)); const m = v - c; const [r, g, b] = h < 1 ? [c, x, 0] : h < 2 ? [x, c, 0] : h < 3 ? [0, c, x] : h < 4 ? [0, x, c] : h < 5 ? [x, 0, c] : [c, 0, x]; return { r: Math.round((r + m) * 255), g: Math.round((g + m) * 255), b: Math.round((b + m) * 255) }; } return null; }
  private formatColor(hex: string): string { const rgb = this.parseColor(hex)!; if (this.format() === 'rgb') return `rgb(${rgb.r}, ${rgb.g}, ${rgb.b})`; if (this.format() === 'hsv' || this.format() === 'hsb') { const r = rgb.r / 255, g = rgb.g / 255, b = rgb.b / 255, max = Math.max(r, g, b), min = Math.min(r, g, b), d = max - min; let h = 0; if (d) h = max === r ? ((g - b) / d) % 6 : max === g ? (b - r) / d + 2 : (r - g) / d + 4; h = Math.round(h * 60); if (h < 0) h += 360; const s = max ? Math.round((d / max) * 100) : 0; return `${this.format() === 'hsb' ? 'hsb' : 'hsv'}(${h}, ${s}%, ${Math.round(max * 100)}%)`; } return hex; }
