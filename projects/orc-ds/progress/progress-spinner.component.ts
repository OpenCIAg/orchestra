...
@Component({
  ...
  template: `<svg class="p-progressspinner p-component orc-progress-spinner" [class]="'p-progressspinner p-component orc-progress-spinner ' + styleClass()" [style]="style()" [class.orc-progress-spinner--static]="animation() === 'none'" [style.width.px]="size()" [style.height.px]="size()" [style.animation-duration]="animationDuration()" role="progressbar" [attr.aria-label]="ariaLabel()" [attr.data-pc-name]="'progressspinner'"><circle class="track" cx="50" cy="50" r="40" [attr.stroke-width]="strokeWidth()" [attr.stroke]="fill()" /><circle class="spinner" cx="50" cy="50" r="40" [attr.stroke-width]="strokeWidth()" [attr.stroke]="color()" /></svg>`,
  styles: [`.orc-progress-spinner{display:inline-block;overflow:visible;animation:orc-spin 1.6s linear infinite}.orc-progress-spinner--static{animation:none}.orc-progress-spinner .track{fill:none;opacity:.2}.orc-progress-spinner .spinner{fill:none;stroke-linecap:round;stroke-dasharray:70 180;transform:rotate(-90deg);transform-origin:50% 50%}@keyframes orc-spin{to{transform:rotate(360deg)}}`],
