...
export class MeterGroupComponent {
  readonly values = input<MeterItem[]>([]); readonly value = input<MeterItem[] | undefined>(undefined); readonly min = input(0); readonly max = input(100); readonly color = input('#3b82f6'); readonly label = input(''); readonly labelPosition = input<'start' | 'end'>('end'); readonly labelOrientation = input<'horizontal' | 'vertical'>('horizontal'); readonly orientation = input<'horizontal' | 'vertical'>('horizontal'); readonly style = input<Record<string, any> | null | undefined>(undefined); readonly styleClass = input(''); readonly ariaLabel = input('Meter');
...
export class SplitButtonComponent { readonly model = input<Array<{ label: string; icon?: string; disabled?: boolean; visible?: boolean; command?: () => void }>>([]); readonly label = input('Action'); readonly icon = input(''); readonly iconPos = input<'left' | 'right'>('left'); readonly style = input<Record<string, any> | null | undefined>(undefined); readonly styleClass = input(''); readonly menuStyle = input<Record<string, any> | null | undefined>(undefined); readonly menuStyleClass = input(''); readonly appendTo = input<unknown>(undefined); readonly expandAriaLabel = input('More actions'); readonly tooltip = input<string | undefined>(undefined); readonly disabled = input(false, { transform: booleanAttribute }); readonly buttonDisabled = input(false, { transform: booleanAttribute }); readonly menuButtonDisabled = input(false, { transform: booleanAttribute }); readonly loading = input(false, { transform: booleanAttribute }); readonly size = input<'small' | 'large' | undefined>(undefined); readonly severity = input<string | undefined>(undefined); readonly autofocus = input(false, { transform: booleanAttribute }); readonly tabindex = input(0); readonly open = model(false); readonly primaryClick = output<Event>(); readonly onClick = this.primaryClick; readonly dropdownClick = output<Event>(); readonly onDropdownClick = this.dropdownClick; readonly onMenuShow = output<void>(); readonly onMenuHide = output<void>();
  ...
  toggleOpen(event?: Event): void { this.open.update(value => !value); if (this.open()) this.onMenuShow.emit(); else this.onMenuHide.emit(); if (event) this.dropdownClick.emit(event); }
}
