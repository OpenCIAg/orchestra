...
@Component({
  ...
  template: `<section class="p-panel p-component orc-p2-panel" [class]="'p-panel p-component orc-p2-panel ' + styleClass()" [style]="style()" [class.collapsed]="collapsed()" [attr.aria-label]="ariaLabel() || null" [attr.data-pc-name]="'panel'">
    <header class="orc-p2-panel__header" (click)="toggle()">
      ...
      @if (toggleable() && (collapsed() ? expandLabel() : collapseLabel())) { <button type="button" class="orc-p2-panel__toggle" [attr.aria-expanded]="!collapsed()" (click)="$event.stopPropagation(); toggle()" [attr.aria-label]="collapsed() ? expandLabel() : collapseLabel()">{{ collapsed() ? '＋' : '−' }}</button> }
    </header>
...
export class PanelComponent {
  ...
  readonly collapsed = model(false); readonly ariaLabel = input(''); readonly expandLabel = input<string | undefined>(undefined); readonly collapseLabel = input<string | undefined>(undefined); readonly onBeforeToggle = output<{ collapsed: boolean }>(); readonly onAfterToggle = output<{ collapsed: boolean }>();
...
@Component({
  ...
  template: `<fieldset class="p-fieldset p-component orc-p2-fieldset" [class]="'p-fieldset p-component orc-p2-fieldset ' + styleClass()" [style]="style()" [class.collapsed]="collapsed()" [attr.aria-label]="ariaLabel() || legend() || null" [attr.data-pc-name]="'fieldset'"><legend>{{ legend() }}@if (toggleable() && (collapsed() ? expandLabel() : collapseLabel())) { <button type="button" [attr.aria-expanded]="!collapsed()" (click)="toggle()" [attr.aria-label]="collapsed() ? expandLabel() : collapseLabel()">{{ collapsed() ? '＋' : '−' }}</button> }</legend>@if (!collapsed()) { <div class="content"><ng-content /></div> }</fieldset>`,
...
export class FieldsetComponent {
  readonly legend = input(''); readonly toggleable = input(false, { transform: booleanAttribute }); readonly collapsed = model(false); readonly styleClass = input(''); readonly style = input<Record<string, string> | null>(null); readonly ariaLabel = input(''); readonly expandLabel = input<string | undefined>(undefined); readonly collapseLabel = input<string | undefined>(undefined); readonly onBeforeToggle = output<{ collapsed: boolean }>(); readonly onAfterToggle = output<{ collapsed: boolean }>();
  toggle(): void { if (!this.toggleable()) return; const next = !this.collapsed(); this.onBeforeToggle.emit({ collapsed: next }); this.collapsed.set(next); this.onAfterToggle.emit({ collapsed: next }); }
  expand(): void { if (this.collapsed()) this.toggle(); }
  collapse(): void { if (!this.collapsed()) this.toggle(); }
}
...
export class MeterGroupComponent {
  readonly values = input<MeterItem[]>([]); readonly value = input<MeterItem[] | undefined>(undefined); readonly min = input(0); readonly max = input(100); readonly color = input('#3b82f6'); readonly label = input(''); readonly labelPosition = input<'start' | 'end'>('end'); readonly labelOrientation = input<'horizontal' | 'vertical'>('horizontal'); readonly orientation = input<'horizontal' | 'vertical'>('horizontal'); readonly style = input<Record<string, any> | null | undefined>(undefined); readonly styleClass = input(''); readonly ariaLabel = input<string | undefined>(undefined);
...
@Component({
  ...
  template: `<div class="orc-p2-password" [class]="styleClass()" [style]="style()" [class.fluid]="fluid()"><label *ngIf="label()" [attr.for]="inputId()">{{ label() }}</label><div class="control"><input [id]="inputId()" [type]="visible() ? 'text' : 'password'" [value]="value()" [attr.placeholder]="placeholder() || null" [autocomplete]="autocomplete()" [attr.maxlength]="maxLength()" [disabled]="disabled() || cvaDisabled()" [readonly]="readonly()" [autofocus]="autofocus()" [attr.tabindex]="tabindex()" [class]="inputStyleClass()" [style]="inputStyle()" (input)="onInput($event)" (focus)="handleFocus($event)" (blur)="handleBlur($event)" [attr.aria-label]="ariaLabel() || null" [attr.aria-labelledby]="ariaLabelledBy()" />@if (showClear() && value() && clearAriaLabel()) { <button type="button" [disabled]="disabled() || cvaDisabled()" (click)="clear()" [attr.aria-label]="clearAriaLabel()">×</button> }@if (toggleMask() && (visible() ? hidePasswordLabel() : showPasswordLabel())) { <button type="button" [disabled]="disabled() || cvaDisabled()" (click)="toggleVisible()" [attr.aria-label]="visible() ? hidePasswordLabel() : showPasswordLabel()">{{ visible() ? '◉' : '○' }}</button> }</div>@if (feedback() && (value() || focused()) && (value() ? strengthLabel() : promptLabel())) { <div class="feedback" aria-live="polite"><span>{{ value() ? strengthLabel() : promptLabel() }}</span><span class="meter" [class]="strength()"></span></div> }</div>`,
...
export class PasswordComponent implements ControlValueAccessor {
  readonly value = model(''); readonly visible = model(false); readonly placeholder = input<string | undefined>(undefined); readonly disabled = input(false, { transform: booleanAttribute }); readonly readonly = input(false, { transform: booleanAttribute }); readonly required = input(false, { transform: booleanAttribute }); readonly ariaLabel = input<string | undefined>(undefined); readonly ariaLabelledBy = input<string | undefined>(undefined); readonly label = input<string | undefined>(undefined); readonly inputId = input<string | undefined>(undefined); readonly inputStyleClass = input(''); readonly inputStyle = input<Record<string, string | number> | undefined>(undefined); readonly styleClass = input(''); readonly style = input<Record<string, string | number> | undefined>(undefined); readonly fluid = input(false, { transform: booleanAttribute }); readonly variant = input<'filled' | 'outlined'>('outlined'); readonly size = input<'small' | 'large' | undefined>(undefined); readonly maxLength = input<number | undefined>(undefined); readonly autocomplete = input('off'); readonly autofocus = input(false, { transform: booleanAttribute }); readonly tabindex = input<number | undefined>(undefined); readonly feedback = input(true, { transform: booleanAttribute }); readonly toggleMask = input(true, { transform: booleanAttribute }); readonly showClear = input(false, { transform: booleanAttribute }); readonly clearAriaLabel = input<string | undefined>(undefined); readonly hidePasswordLabel = input<string | undefined>(undefined); readonly showPasswordLabel = input<string | undefined>(undefined); readonly appendTo = input<unknown>(undefined); readonly showTransitionOptions = input('150ms ease'); readonly hideTransitionOptions = input('150ms ease'); readonly promptLabel = input<string | undefined>(undefined); readonly weakLabel = input<string | undefined>(undefined); readonly mediumLabel = input<string | undefined>(undefined); readonly strongLabel = input<string | undefined>(undefined); readonly mediumRegex = input('^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d).{6,}$'); readonly strongRegex = input('^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[^A-Za-z\\d]).{8,}$');
...
export class SplitButtonComponent { readonly model = input<Array<{ label: string; icon?: string; disabled?: boolean; visible?: boolean; command?: () => void }>>([]); readonly label = input<string | undefined>(undefined); readonly icon = input(''); readonly iconPos = input<'left' | 'right'>('left'); readonly style = input<Record<string, any> | null | undefined>(undefined); readonly styleClass = input(''); readonly menuStyle = input<Record<string, any> | null | undefined>(undefined); readonly menuStyleClass = input(''); readonly appendTo = input<unknown>(undefined); readonly expandAriaLabel = input<string | undefined>(undefined); readonly tooltip = input<string | undefined>(undefined); readonly disabled = input(false, { transform: booleanAttribute }); readonly buttonDisabled = input(false, { transform: booleanAttribute }); readonly menuButtonDisabled = input(false, { transform: booleanAttribute }); readonly loading = input(false, { transform: booleanAttribute }); readonly closeOnEscape = input(true, { transform: booleanAttribute }); readonly size = input<'small' | 'large' | undefined>(undefined); readonly severity = input<string | undefined>(undefined); readonly autofocus = input(false, { transform: booleanAttribute }); readonly tabindex = input(0); readonly open = model(false); readonly primaryClick = output<Event>(); readonly onClick = this.primaryClick; readonly dropdownClick = output<Event>(); readonly onDropdownClick = this.dropdownClick; readonly onMenuShow = output<void>(); readonly onMenuHide = output<void>();
  effectiveModel(): Array<{ label: string; icon?: string; disabled?: boolean; visible?: boolean; command?: () => void }> { return this.model(); }
  activate(item: { command?: () => void }): void { item.command?.(); this.toggleOpen(); }
  onDefaultButtonClick(event: MouseEvent): void { if (!this.disabled() && !this.buttonDisabled() && !this.loading()) this.primaryClick.emit(event); }
  toggleOpen(event?: Event): void { if (this.disabled() || this.menuButtonDisabled() || this.loading()) return; this.open.update(value => !value); if (this.open()) this.onMenuShow.emit(); else this.onMenuHide.emit(); if (event) this.dropdownClick.emit(event); }
  @HostListener('document:keydown.escape') onEscape(): void { if (this.open() && this.closeOnEscape()) { this.open.set(false); this.onMenuHide.emit(); } }
}

@Component({
  selector: 'orc-scroll-top', standalone: true,
  template: `@if (visible()) { <button type="button" class="p-scrolltop p-component orc-p2-scroll-top" [class]="'p-scrolltop p-component orc-p2-scroll-top ' + styleClass()" [style]="style()" [attr.aria-label]="buttonAriaLabel() || ariaLabel() || null" [attr.data-pc-name]="'scrolltop'" (click)="scroll()">{{ icon() }}</button> }`,
