...
@Component({
  ...
  template: `<section class="orc-messages" [attr.aria-label]="ariaLabel()" [attr.aria-live]="ariaLive()">@for (message of messages(); track message.id ?? $index) { <article class="message" [class]="'severity-' + (message.severity || 'info')" role="alert"><div>@if (message.summary) { <strong>{{ message.summary }}</strong> } @if (message.detail) { <span>{{ message.detail }}</span> }</div>@if (closable() && message.closable !== false) { <button type="button" [attr.aria-label]="closeLabel()" (click)="remove($index)">×</button> }</article> }</section>`,
