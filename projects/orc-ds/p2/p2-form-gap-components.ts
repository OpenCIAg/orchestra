...
@Component({
  ...
  template: `<button type="button" class="orc-toggle-button" [class.checked]="checked()" [class]="styleClass()" [disabled]="disabled()" [attr.id]="inputId()" [attr.tabindex]="tabindex()" [autofocus]="autofocus()" [attr.aria-label]="ariaLabel()" [attr.aria-labelledby]="ariaLabelledBy()" [attr.aria-pressed]="checked()" (click)="toggle($event)">{{ checked() ? onIcon() : offIcon() }} {{ checked() ? onLabel() : offLabel() }}<ng-content /></button>`,
