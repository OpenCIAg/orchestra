...
@Component({
  ...
  template: `<div class="p-selectbutton p-buttonset p-component orc-p2-segmented" [class]="'p-selectbutton p-buttonset p-component orc-p2-segmented ' + styleClass()" [style]="style()" [attr.id]="inputId() || null" role="group" [attr.aria-label]="label()" [attr.data-pc-name]="'selectbutton'" (keydown)="onKeydown($event)">@for (option of options(); track option.value) { <button type="button" class="p-button p-component" [disabled]="option.disabled || disabled() || cvaDisabled() || readonly()" [attr.tabindex]="tabindex()" [class.selected]="isSelected(option)" [attr.aria-pressed]="isSelected(option)" (click)="select(option)">{{ option.icon }} {{ option.label }}</button> }</div>`,
...
@Component({
  ...
  template: `
    <div class="p-treeselect p-component orc-p2-tree-select" [class]="'p-treeselect p-component orc-p2-tree-select ' + styleClass()" [style]="style()" [class.p-treeselect-fluid]="fluid()" [attr.data-pc-name]="'treeselect'">
      ...
      @if (showClear() && value() !== null && !readonly()) { <button type="button" (click)="clear($event)" aria-label="Clear">×</button> }
      @if (open()) {
