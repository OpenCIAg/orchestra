...
export class TreeSelectComponent implements ControlValueAccessor {
  ...
  toggleOpen(): void { if (this.disabled() || this.cvaDisabled()) return; this.open.update(value => !value); if (!this.open() && this.resetFilterOnHide()) this.filterValue.set(''); this.open() ? this.onShow.emit() : this.onHide.emit(); }
  ...
  clear(event: Event): void { if (this.disabled() || this.cvaDisabled()) return; this.value.set(null); this.onModelChange(null); this.onModelTouched(); this.onClear.emit(event); }
