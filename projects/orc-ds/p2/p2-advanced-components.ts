...
export class TieredMenuComponent {
  readonly items = input<PrimeMenuItem[]>([]); readonly model = input<PrimeMenuItem[] | undefined>(undefined); readonly style = input<Record<string, any> | null | undefined>(undefined); readonly styleClass = input(''); readonly appendTo = input<HTMLElement | string | null | undefined>(undefined); readonly breakpoint = input(''); readonly autoZIndex = input(true, { transform: booleanAttribute }); readonly baseZIndex = input(0); readonly autoDisplay = input(true, { transform: booleanAttribute }); readonly showTransitionOptions = input(''); readonly hideTransitionOptions = input(''); readonly ariaLabel = input('Menu'); readonly ariaLabelledBy = input<string | undefined>(undefined); readonly id = input<string | undefined>(undefined); readonly disabled = input(false, { transform: booleanAttribute }); readonly popup = input(false, { transform: booleanAttribute }); readonly tabindex = input(0); readonly visible = model(false); readonly openItem = signal<PrimeMenuItem | null>(null); readonly activeIndex = signal(0); readonly itemSelect = output<PrimeMenuItem>(); readonly onItemClick = output<PrimeMenuItem>(); readonly onShow = output<void>(); readonly onHide = output<void>(); readonly onFocus = output<Event>(); readonly onBlur = output<Event>();
...
export class PanelMenuComponent {
  readonly items = input<PrimeMenuItem[]>([]); readonly model = input<PrimeMenuItem[] | undefined>(undefined); readonly style = input<Record<string, any> | null | undefined>(undefined); readonly styleClass = input(''); readonly id = input<string | undefined>(undefined); readonly transitionOptions = input(''); readonly tabindex = input(0); readonly ariaLabel = input('Panel menu'); readonly disabled = input(false, { transform: booleanAttribute }); readonly multiple = input(false, { transform: booleanAttribute }); readonly open = model<ReadonlySet<PrimeMenuItem>>(new Set()); readonly itemSelect = output<PrimeMenuItem>(); readonly onItemExpand = output<PrimeMenuItem>(); readonly onItemCollapse = output<PrimeMenuItem>(); readonly onNodeSelect = this.itemSelect; readonly onNodeExpand = this.onItemExpand; readonly onNodeCollapse = this.onItemCollapse;
  effectiveItems(): PrimeMenuItem[] { return this.model() ?? this.items(); }
...
@Component({
  ...
  template: `<section class="orc-data-view" [class]="'orc-data-view ' + styleClass()" [style]="style()" [attr.aria-label]="ariaLabel()">@if (header()) { <header>{{ header() }}</header> }@if (filterBy()) { <input [value]="filterValue()" (input)="filterValue.set($any($event.target).value)" aria-label="Filter data view" /> }<div class="content" [class.list]="layout() === 'list'" [class]="layout() === 'list' ? listStyleClass() : gridStyleClass()">@if (loading()) { <p>{{ loadingIcon() || 'Loading…' }}</p> } @else { @for (item of pageItems(); track getItemKey(item, $index)) { <article>@if (itemTemplate()) { <ng-container [ngTemplateOutlet]="itemTemplate()" [ngTemplateOutletContext]="{ $implicit: item }" /> } @else { {{ itemLabel(item) }} }</article> } @empty { <p>{{ emptyMessage() }}</p> } }</div>@if (paginator() && pageCount() > 1) { <nav class="paginator" aria-label="Data view pages"><button type="button" [disabled]="first() === 0" (click)="goToPage(first() - rows())">‹</button><span>{{ (first() / rows()) + 1 }} / {{ pageCount() }}</span><button type="button" [disabled]="first() + rows() >= effectiveTotalRecords()" (click)="goToPage(first() + rows())">›</button></nav> }</section>`,
  imports: [CommonModule],
...
export class DataViewComponent<T = Record<string, unknown>> implements OnInit {
  readonly value = input<T[]>([]); readonly layout = model<'list' | 'grid'>('grid'); readonly header = input(''); readonly emptyMessage = input('No items'); readonly ariaLabel = input('Data view'); readonly itemTemplate = input<any>(null); readonly style = input<Record<string, string | number> | undefined>(undefined); readonly styleClass = input(''); readonly gridStyleClass = input(''); readonly listStyleClass = input(''); readonly trackBy = input<((index: number, item: T) => unknown) | undefined>(undefined);
